...
settings["global"]["experiment_name"] = "VotingPrediction"
...
dfg.setup(ts_storage, TFList(tfs), settings, "DFG-NO_WT_LambdaW-{0}".format(0.1), 20, None, None, None, False)
jobman.queueJob(dfg)
...
votejob = CLR()
votejob.setup(cnlo_storage, settings, "Voting")
jobman.queueJob(votejob)
votejob = jobman.queue[0]

results = []

votenet = VotingNetwork(jobman.finished, dfg.gene_list)

dfg.predict_from_weights(votenet, dfg.settings)
