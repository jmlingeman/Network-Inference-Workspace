...
PARAMS[["general"]][["numBoots"]] = {{bootstrap_runs}}
...
PARAMS[["general"]][["delT_max"]] = {{delta_t_max}} # for dream4 try 110-210, for dream3 try 45-85, for dream2 try 25-45 (ie consider at most two-four consecutive steps in time series)
...
PARAMS[["lars"]][["lambda"]] = c(0)
PARAMS[["general"]][["tau"]] = {{tau}} # for dream4 try 40-60, for dream3 15-25, for dream2 try ~10 (ie assumes most reactions happen in about a time step)
...
PARAMS[["general"]][["processorsNumber"]] = {{num_procs}}
...
PARAMS[["general"]][["plot.it"]] <- {{plot}}
PARAMS[["general"]][["data"]] <- "{{name}}"
PARAMS[["general"]][["output_file"]] <- "../output/inferelator_output.csv"
#############################################################################
...
PARAMS[["clr"]][["response_matrix"]] = '{{clr_response}}'
PARAMS[["lars"]][["response_matrix"]] = '{{lars_response}}'

#x = let_usr_choose_design_matrix()
PARAMS[["clr"]][["design_matrix"]] = '{{clr_design}}'
PARAMS[["lars"]][["design_matrix"]] = '{{lars_design}}'
...
if ('{{timeseries}}' != 'None') {
    ts_filename = '{{timeseries}}'
} else {
    ts_filename = ''
}
print("Loading KO data\n")
if ('{{knockout}}' != 'None') {
    ko_filename = '{{knockout}}'
} else {
    ko_filename = ''
}
...
numEq_exp <- {{num_steadystate_exp}} #the number of non-time-series experiments
...
obj = list()
tfNames <- load_tfnames('{{tfs_filename}}', ratios)
obj[[1]] = ratios
obj[[2]] = clusterStack
obj[[3]] = colMap
obj[[4]] = tfNames
