...
for dataset in [kno_ts_data, pert_data, ss_data, pert_baseline]:
    for exp in dataset.experiments:
        for gene in dataset.gene_list:
            exp.ratios[gene] = exp.ratios[gene] - min_value
            if exp.ratios[gene] < 0.001:
...
inf.setup(None, ss_data, settings, timeseries_data, pert_data, "Inferelator2_TS_SS".format(meth, "inf_test"), None, prior=genie3job.network, split_ts=False, leave_outs=leave_out)
...
settings["inferelator2"]["method"] = meth
settings["global"]["time_series_delta_t"] = 3.0
inf.setup(None, ss_data, settings, timeseries_data, pert_data, "Inferelator2_TS_SS_NOPRIOR".format(meth, "inf_test"), None, prior=None, split_ts=False, leave_outs=leave_out)
