...
exp_data_directory = 'experiments/ExperimentType/data'
# timeseries_filename = exp_set + '-1_nonoise_dream4_timeseries.tsv'
# wildtype_filename = exp_set + '-1_nonoise_wildtype.tsv'
# knockout_filename = exp_set + '-1_nonoise_knockouts.tsv'
# knockdown_filename = exp_set + '-1_nonoise_knockdowns.tsv'
# multifactorial_filename = exp_set + '-1_noexpnoise_multifactorial.tsv'
# dex_filename = exp_set + '-1_nonoise_multifactorial.tsv'

timeseries_filename = exp_set + '-1_dream4_timeseries.tsv'
wildtype_filename = exp_set + '-1_wildtype.tsv'
knockout_filename = exp_set + '-1_knockouts.tsv'
knockdown_filename = exp_set + '-1_knockdowns.tsv'
...
ts_pert_data["knockout_data"] = ReadData(exp_data_directory + "/" + exp_set + "/" + '/TS/' + knockout_filename, "knockout")
...
mczjob = MCZ()
mczjob.setup(ko_pert_data["knockout_data"], ko_pert_data["ss_data"], settings, None, None, "MCZ-KO")
jobman.queueJob(mczjob)
...
# 1 run is 11 TS, N KO, M-TS multifactorial

# Another run is N KO, M multifactorial

# Another run is KO

# Another run is Multifactorial
...
settings["inferelator2"]["delta_t_max"] = 200
settings["inferelator2"]["delta_t_min"] = 0
settings["inferelator2"]["tau"] = 25
...
settings["global"]["time_series_delta_t"] = 100.0
inf.setup(ts_only_data["multifactorial_data"], ts_only_data["ss_data"], settings, ts_only_data["timeseries"], ts_only_data["knockout_data"], "Inferelator2_TS_KO_Mult_Prior", None, prior=both_genie3.network, split_ts=True, leave_outs=None)
...
settings["inferelator2"]["method"] = meth
settings["global"]["time_series_delta_t"] = 100.0
inf.setup(ts_only_data["multifactorial_data"], ts_only_data["ss_data"], settings, ts_only_data["timeseries"], ts_only_data["knockout_data"], "Inferelator2_TS_KO_Mult", None, prior=None, split_ts=True, leave_outs=None)
#inf.setup(None, ss_data, settings, [kcl_ts_data], None, "Inferelator2_TS_SS".format(meth, "inf_test"), prior=genie3job.network, split_ts=False, gold_std=None, leave_outs=leave_out)
jobman.queueJob(inf)
