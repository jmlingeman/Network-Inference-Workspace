...
settings["global"]["experiment_name"] = "GENIE3_Error_Bars_New_Noise_" + sys.argv[1]

#settings["global"]["n_processors"] = multiprocessing.cpu_count()-2
...
for name in data.keys():
    for i in range(50):
        ...
        genie3job = GENIE3()
        genie3job.setup(combined, settings, "Genie3_TimeSeries_Only_{0}-{1}".format(name, i))
        jobman.queueJob(genie3job)
        genie3nets[name] = genie3job
...
for name in data.keys():
    for i in range(50):
        ...
        genie3job = GENIE3()
        genie3job.setup(combined, settings, "Genie3_TimeSeries_KO_{0}-{1}".format(name, i))
        jobman.queueJob(genie3job)
        genie3nets[name] = genie3job
...
for name in data.keys():
    for i in range(50):
        ...
        genie3job = GENIE3()
        genie3job.setup(combined, settings, "Genie3_TimeSeries_KO_SS_{0}-{1}".format(name, i))
        jobman.queueJob(genie3job)
        genie3nets[name] = genie3job
...
#for name in data.keys():
    ...
    #for i in range(1):
            ...
            #inf.setup(None, None, settings, ts_storage, multifactorials[name], "Inferelator2_{0}".format(name, meth), goldnet, genie3nets[name].network)
            #jobman.queueJob(inf)
