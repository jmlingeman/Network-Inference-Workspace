...
settings["global"]["experiment_name"] = "NoiseModel_" + sys.argv[1]
...
for name in data.keys():
    ...
    for i in range(1):
        ...
        for meth in ["MEN"]:
            ...
            inf.setup(None, wildtypes[name], settings, ts_storage, None, "Inferelator2_{0}".format(name, meth), None)
            jobman.queueJob(inf)
...
for job in jobman.finished:
    if "12" in job.alg.name:
        high_rep.append(job)
    elif "4" in job.alg.name:
        low_rep.append(job)
    else:
        med_rep.append(job)
