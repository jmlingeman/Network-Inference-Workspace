...
settings["global"]["experiment_name"] = "BestOfBreed"
...
#print settings["global"]["time_series_delta_t"]
#dfg.setup(ts_storage, TFList(tfs[exp_name]), settings, "DFG-{1}_LambdaW-{0}".format(0.1, exp_name), 20)
#jobman.queueJob(dfg)
percfp = 0
permfp = 1
perctp = 0
...
settings["inferelator2"]["percfp"] = percfp
settings["inferelator2"]["method"] = meth
inf.setup(knockouts[exp_name], wildtypes[exp_name], settings, ts_storage, None, "Inferelator2-KO-TS-{0}-{1}".format(meth, exp_name), None)
jobman.queueJob(inf)
...
settings["inferelator2"]["percfp"] = percfp
settings["inferelator2"]["method"] = meth
inf.setup(knockouts[exp_name], wildtypes[exp_name], settings, ts_storage, knockdowns[exp_name], "Inferelator2-Both-{0}-{1}".format(meth, exp_name), None)
jobman.queueJob(inf)


#genie3job = GENIE3()
#genie3job.setup(multifactorials[exp_name], settings, "Genie3-Multifactorial")
#jobman.queueJob(genie3job)

#mczjob = MCZ()
#mczjob.setup(knockouts[exp_name], wildtypes[exp_name], settings, None, None, "MCZ-Knockout")
...
##settings["inferelator2"]["num_bootstraps"] = 1
    ...
    #settings["inferelator2"]["permfp"] = permfp
    #settings["inferelator2"]["nCv"] = 8
    #settings["inferelator2"]["prior_weight"] = 1.0
    #settings["inferelator2"]["perctp"] = perctp
    #settings["inferelator2"]["percfp"] = percfp
    #settings["inferelator2"]["method"] = meth
    #inf.setup(knockouts[exp_name], wildtypes[exp_name], settings, ts_storage, multifactorials[exp_name], "Inferelator2-Combined-{0}-{1}-{2}".format(meth, exp_name, prior[0]), None, prior[1])
    #jobman.queueJob(inf)

    ##meth = "BBSR"
    ##inf = Inferelator2(settings)
    ##settings["inferelator2"]["num_cores"] = 3
    ##settings["inferelator2"]["num_bootstraps"] = 50
###settings["inferelator2"]["num_bootstraps"] = 1
    ##settings["inferelator2"]["permtp"] = permtp
    ##settings["inferelator2"]["permfp"] = permfp
    ...
    ##settings["inferelator2"]["percfp"] = percfp
    ##settings["inferelator2"]["method"] = meth
    ##inf.setup(knockouts[exp_name], wildtypes[exp_name], settings, ts_storage, multifactorials[exp_name], "Inferelator2-Combined-{0}-{1}-{2}".format(meth, exp_name, prior[0]), goldnet, prior[1])
    ##jobman.queueJob(inf)
