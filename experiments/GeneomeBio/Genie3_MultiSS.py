...
if sys.argv[1] == "100":
    ...
    exp_data_directory = 'datasets/20Genes-SS/' + fold + '/'
...
settings["global"]["experiment_name"] = "GENIE3_SS-Only_" + fold+ "_" + sys.argv[1]
...
genie3job = GENIE3()
genie3job.setup(combined, settings, "Genie3_{0}_Steady_State_Only".format(fold.capitalize()))
...
ps, rs, precs = GenerateMultiPR(jobman.finished, goldnet, False, settings["global"]["output_dir"] + "/OverallPR.pdf", True, "{0} Genes - {1} - Steady State Only".format(len(goldnet.network), fold))
