...
##for i in range(1):
    ...
    #settings["inferelator2"]["delta_t_max"] = delta
...
##settings["inferelator2"]["num_bootstraps"] = 1
    ...
    #inf.setup(knockouts[name], None, settings, ts_storage, None, "Inferelator2_{0}-Tau{1}-Delta{2}".format(name, tau, delta), goldnet, genie3nets[name].network)
    #jobman.queueJob(inf)
...
##settings["inferelator2"]["num_bootstraps"] = 1
    ...
    #settings["inferelator2"]["percfp"] = percfp
    #settings["inferelator2"]["method"] = meth
    #inf.setup(knockouts[name], None, settings, ts_storage, None, "Inferelator2_{0}-Tau{1}-Delta{2}-HWT".format(name, tau, delta), goldnet, genie3nets[name].network)
    #jobman.queueJob(inf)
...
for name in data.keys():
    ...
    #taus = [10,20,45,65,85]
    #deltas = [25,30,45,65,85,110]
    #delta_mins = [0,5,10,15,20]

    #for i in range(len(taus)):
        #for j in range(len(deltas)):
            #for k in range(len(delta_mins)):
    tau = 60
    delta = 60
    delta_min = 15
    ...
    permtp = 1
    inf = Inferelator2(settings)
    settings["inferelator2"]["num_cores"] = multiprocessing.cpu_count() - 6

    settings["inferelator2"]["tau"] = tau
    settings["inferelator2"]["delta_t_max"] = delta
    settings["inferelator2"]["delta_t_min"] = delta_min
    settings["inferelator2"]["num_bootstraps"] = 15
    settings["inferelator2"]["prior_weight"] = 1.1
    #settings["inferelator2"]["num_bootstraps"] = 1
    if sys.argv[1] == "20small":
        settings["inferelator2"]["nCv"] = 10
    ...
    settings["inferelator2"]["percfp"] = percfp
    settings["inferelator2"]["method"] = meth
    inf.setup(knockouts[name], None, settings, ts_storage, None, "Inferelator2_{0}-Tau{1}-DeltaMax{2}-DeltaMin{3}".format(name, tau, delta, delta_min), goldnet, genie3nets[name].network)
    jobman.queueJob(inf)
...
jobman.waitToClear()
    ...
    #print settings["global"]["time_series_delta_t"]
    #dfg.setup(ts_storage, TFList(tfs[name]), settings, "DFG-{1}".format(name), 20)
    #jobman.queueJob(dfg)
