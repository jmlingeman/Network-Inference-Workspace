...
settings["global"]["experiment_name"] = "Christoph_NoiseModel_TS-SS-KO_MP_MEN_" + sys.argv[1]
...
#for name in data.keys():
print data.keys()
for name in [sys.argv[1] + "Gene-12rep-5t"]:
    ts_storage = data[name]
    settings["global"]["time_series_delta_t"] = (200.0 / (len(ts_storage[0].experiments)-1))
...
#settings["inferelator2"]["num_bootstraps"] = 3
    ...
    inf.setup(knockouts[name], wildtypes[name], settings, ts_storage, knockdowns[name], "Inferelator2_{0}_{1}_NoPrior".format(meth, name), goldnet, genie3nets[name].network)
    jobman.queueJob(inf)
...
#settings["inferelator2"]["num_bootstraps"] = 3
    ...
    settings["inferelator2"]["percfp"] = percfp
    settings["inferelator2"]["method"] = meth
    settings["inferelator2"]["prior_weight"] = 0.8
    inf.setup(knockouts[name], wildtypes[name], settings, ts_storage, knockdowns[name], "Inferelator2_{0}_{1}".format(meth, name), goldnet, genie3nets[name].network)
    jobman.queueJob(inf)
