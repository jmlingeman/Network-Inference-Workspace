...
class InferelatorPipeline(Algorithm):
    def __init__(self):

        #os.chdir("/Users/Jesse/Workspace/School/MastersThesis/Program")
        self.alg_name = "inferelator-pipeline"
    ...
    def setup(self, ko_storage, wt_storage, settings, ts_storage=None, other_storage=None, name=None):
      ...
      if name == None:
          self.name = "inferelator-pipeline-" + datetime.now().strftime("%Y-%m-%d_%H.%M.%S")
      else:
          self.name = name

      settings = ReadConfig(settings, "./config/default_values/inferelator-pipeline.cfg")
      ...
      if ts_storage != None:
        ...
        settings["inferelator-pipeline"]["timeseries"] = "None"
      if ko_storage != None:
          self.write_data(ko_storage, settings, "knockout")
          num_ko_exp = len(ko_storage.experiments)
      else:
          settings["inferelator-pipeline"]["knockout"] = "None"
          num_ko_exp = 0
      ...
      if wt_storage != None:
        ...
        num_wt_exp = len(wt_storage.experiments)
          ...
          settings["inferelator-pipeline"]["wildtype"] = "None"
          num_wt_exp = 0
      ...
      if other_storage != None:
          ...
          settings["inferelator-pipeline"]["other"] = "None"
      settings[self.alg_name]["num_steadystate_exp"] = num_steadystate_exp
      self.settings = settings
      self.write_config(settings)

      self.cmd = "Rscript r_scripts/main.R"
      #self.cmd = "python inferelator.py {0} {1} {2}".format(settings["inferelator-pipeline"]["wildtype"], settings["inferelator-pipeline"]["knockout"], settings["inferelator-pipeline"]["output_dir"] + "/output/inferelator_output.txt")
      print self.cmd
      #exit()
    ...
    def write_config(self, settings):
      ...
      WriteConfig("inferelator-pipeline", settings)
      ...
      if os.path.isfile(settings["inferelator-pipeline"]["working_dir_config"]) or \
              os.path.islink(settings["inferelator-pipeline"]["working_dir_config"]):
                  os.remove(settings["inferelator-pipeline"]["working_dir_config"])
      os.symlink( settings["inferelator-pipeline"]["settings_file"], \
              settings["inferelator-pipeline"]["working_dir_config"])
    ...
    def write_data(self, storage, settings, filetype):
       ...
       if filetype == "timeseries":
        settings["inferelator-pipeline"][filetype] = filename = self.data_dir \
    ...
    def read_data(self, settings):
      ...
      self.exp_data.read_input(settings["inferelator-pipeline"]["ratio_file"])
...
if __name__ == "__main__":
    ...
    usage = "Usage: python inferelator_pipeline.py --name NAME -g goldnetworkfile -t timeseriesfile -k knockoutfile steadystate_datafile1 steadystate_datafile2 ..."
