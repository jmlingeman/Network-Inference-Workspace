...
class TLCLR(Algorithm):
    ...
    def setup(self, ko_storage, wt_storage, settings, ts_storage=None, other_storage=None, name=None):
      ...
      """ This function sets up TLCLR based on the input parameters above.
       For setting something up with just a config file, use setup_config """
      if name == None:
          self.name = "tlclr-" + datetime.now().strftime("%Y-%m-%d_%H.%M.%S")
      else:
          self.name = name

      settings = ReadConfig(settings, "./config/default_values/tlclr.cfg")
      settings = ReadConfig(settings, settings["tlclr"]["config"])
      ...
      if wt_storage != None:
        ...
        num_wt_exp = len(wt_storage.experiments)
          ...
          settings["tlclr"]["wildtype"] = "None"
          num_wt_exp = 0
      self.gene_list = ts_storage[0].gene_list

      # Read in the default values for tlclr, we'll override these later
      ...
      if other_storage != None:
          ...
          settings["tlclr"]["other"] = "None"
      settings[self.alg_name]["num_steadystate_exp"] = num_steadystate_exp
      self.settings = settings
      self.write_config(settings)

      self.cmd = "Rscript r_scripts/main.R"
      #self.cmd = "python tlclr.py {0} {1} {2}".format(settings["tlclr"]["wildtype"], settings["tlclr"]["knockout"], settings["tlclr"]["output_dir"] + "/output/tlclr_output.txt")
      print self.cmd
      #exit()
      self.cwd = self.working_dir

    def write_config(self, settings):
        # Code to write the config file on the fly before calling tlclr
      # This will allow us to use the stock implementation of tlclr
      WriteConfig("tlclr", settings)

      # Now we have to write a symlink to this file into the tlclr
       # directory.  This is because tlclr must be run from its directory
       # because it relies on relative paths.

      if os.path.isfile(settings["tlclr"]["working_dir_config"]) or \
              os.path.islink(settings["tlclr"]["working_dir_config"]):
                  os.remove(settings["tlclr"]["working_dir_config"])
      os.symlink( settings["tlclr"]["settings_file"], \
              settings["tlclr"]["working_dir_config"])

    def write_data(self, storage, settings, filetype):
       # This will write out the data files that tlclr is going to read in
       # and set up the input folder.

       import WriteData
       if filetype == "timeseries":
        settings["tlclr"][filetype] = filename = self.data_dir \
    ...
    def read_output(self,settings):
      ...
      output_file = self.output_dir + "/output/tlclr_output.csv"
    ...
    def read_data(self, settings):
      ...
      self.exp_data.read_input(settings["tlclr"]["ratio_file"])
    ...
    def run(self, kofile, tsfile, wtfile, datafiles, name, goldnet_file, normalize=False):
        ...
        # Make TLCLR jobs
        tlclrjob = TLCLR()
        tlclrjob.setup(knockout_storage, wildtype_storage, settings, timeseries_storage, other_storage, name)
...
if __name__ == "__main__":
    ...
    usage = "Usage: python tlclr.py --name NAME -g goldnetworkfile -t timeseriesfile -k knockoutfile steadystate_datafile1 steadystate_datafile2 ..."
    ...
    tlclr = TLCLR()
    tlclr.run(options.kofile, options.tsfile, options.wtfile, args, options.name, options.goldnet_file)
