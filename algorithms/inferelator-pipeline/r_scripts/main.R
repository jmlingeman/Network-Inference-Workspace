...
##  .-.-.***.-.-.***.-.-.***.-.-.***.-.-.***.-.-.***.-.-.***.-.-.
	...
	source("./inferelator-pipeline_launch_script.R")
...
# 13- making predictions of response of the system to double knocouts based on the median of the betas from the ensemble
    ...
    write.table(median.conf.scores, file=PARAMS$general$output_file, sep="\t", col.names=T, row.names=T)
    #write.table(medBeta, file=PARAMS$general$output_file, sep="\t", col.names=T, row.names=T)
##  .-.-.***.-.-.***.-.-.***.-.-.***.-.-.***.-.-.***.-.-.***.-.-.
# 14- print results
##  .-.-.***.-.-.***.-.-.***.-.-.***.-.-.***.-.-.***.-.-.***.-.-.
