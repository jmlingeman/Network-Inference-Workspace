...
makeRatios <- function(chalName, ts_filename, ko_filename, kd_filename, wt_filename){

	#we first load each of the four different datasets
    if(ts_filename != '') {
    print("Reading timeseries file...")
        print(ts_filename)
        dream_ts <- t(read.table(file=paste( ts_filename, sep=""),sep="\t",header=T))
        print("Getting col names for timeseries...")
    ...
    if(ko_filename != '') {
    print("Reading knockout file...")
    dream_ko <- t(read.table(file=paste(ko_filename, sep=""),sep="\t",header=T))
	colnames(dream_ko) <- paste(rownames( dream_ts ),"(-/-)",sep="")
    } else {
    dream_ko = NULL
    }
    if(kd_filename != '') {
    print("Reading knockdown file...")
    dream_kd <- t(read.table(file=paste(kd_filename, sep=""),sep="\t",header=T))
	colnames(dream_kd) <- paste(rownames( dream_ts ),"(+/-)",sep="")
    } else {
    dream_kd = NULL
    }
    if(wt_filename != '') {
    print("Reading wildtype file...")
    dream_wt <- as.matrix(read.table(file=paste(wt_filename, sep=""),sep="\t",header=T))
    } else {
    dream_wt = NULL
    }
