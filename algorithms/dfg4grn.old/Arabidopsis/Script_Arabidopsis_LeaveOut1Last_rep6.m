...
yTrain{1} = KNO3norm{1}(:, 1:2);
yTrain{2} = KNO3norm{2}(:, 1:2);
yTrain{3} = KNO3norm{3}(:, 1:2);
yTrain{4} = KNO3norm{4}(:, 1:2);
yTrain{5} = KNO3norm{1}(:, 4:6);
yTrain{6} = KNO3norm{2}(:, 4:6);
yTrain{7} = KNO3norm{3}(:, 4:6);
yTrain{8} = KNO3norm{4}(:, 4:6);
for k = 1:n_seq_train
  knownTrain{k} = ones(size(yTrain{k}));
  deltaTtrain{k} = [3];
end
for k = 1:n_seq_test
  yTest{k} = KNO3norm{k}(:, 6:7);
